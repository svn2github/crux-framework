 ...
 *
 */
public class ImageResourcesRebind
{
	private final View view;
	private final Device device;
	private final Resources resources;

	ImageResourcesRebind(View view, Device device, Resources resources)
    {
		this.view = view;
		this.device = device;
		...
		while (images.hasNext())
		{
			...
			if (image != null)
			{
				...
				if (image.isFlipRtl() || image.getRepeatStyle() != null || image.isPreventInlining() || image.getWidth() > -1 || image.getHeight() > -1)
				{
					...
					if (image.getWidth() > -1)
					{
						srcWriter.print(", width="+image.getWidth());
					}
					if (image.getHeight() > -1)
					{
						srcWriter.print(", height="+image.getHeight());
					}
					srcWriter.println(")");
				}
