 ...
 *
 */
public class AttachEvtBind implements EvtBinder<Widget>
{
	...
	{
		final Event eventAttach = EvtBind.getWidgetEvent(element, EVENT_NAME);
		if (eventAttach != null)
		{
			widget.addAttachHandler(new Handler()
			{
				public void onAttachOrDetach(AttachEvent event)
				{
					if (event.isAttached())
					{
						Events.callEvent(eventAttach, event);
					}
				}
			});
		}
